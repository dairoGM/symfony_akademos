...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:fo="http://www.w3.org/1999/XSL/Format">
    ...
    <xsl:decimal-format name="kwanzas" decimal-separator="," grouping-separator="." zero-digit="0"/>
    <xsl:decimal-format name="USD" decimal-separator="." grouping-separator="," zero-digit="0"/>
    <xsl:template match="root">
        <fo:root>
            <!-- Definición de layouts para las páginas-->
            <fo:layout-master-set>
                <fo:simple-page-master master-name="Portada" page-height="11.69in" page-width="8.27in" margin-top="0.5in" margin-bottom="0.15in" margin-left="0.65in" margin-right="0.35in">
                    <fo:region-body margin-top="0in" margin-bottom="0.5in" />
                    <fo:region-after extent="0.38in" />
                </fo:simple-page-master>
                <fo:simple-page-master master-name="PaginasIzquierdas" page-height="11.69in" page-width="8.27in" margin-top="0.5in" margin-bottom="0.15in" margin-left="0.65in" margin-right="0.35in">
                    <fo:region-body margin-top="0.7in" margin-bottom="0.5in" />
                    <fo:region-before extent="0.43in" />
                    <fo:region-after extent="0.38in" />
                </fo:simple-page-master>
                <fo:simple-page-master master-name="PaginasDerechas" page-height="11.69in" page-width="8.27in" margin-top="0.5in" margin-bottom="0.15in" margin-left="0.65in" margin-right="0.35in">
                    <fo:region-body margin-top="0.7in" margin-bottom="0.5in" />
                    <fo:region-before extent="0.43in" />
                    <fo:region-after extent="0.38in" />
                </fo:simple-page-master>
                ...
                <fo:flow flow-name="xsl-region-body">
                    <fo:block font-family="MyriadWebPro, sans-serif" font-size="11pt">
                        ...
                        <xsl:if test="data/element">
                            <fo:table margin-top="2em" table-omit-footer-at-break="true" table-omit-header-at-break="false" width="100%" block-progression-dimension="auto" table-layout="fixed" border-style="solid" border-width="0.007in" border-color="#a0a0a0" text-align="left" font-size="9pt" color="#3d3d3d">
                                         
                                <!--Columnas dinamicas -->
                                <xsl:for-each select="//header/element">
                                ...
                                <fo:table-header>
                                    <fo:table-row font-size="11pt" font-weight="bold" height="20px" line-height="20px"  border-top-width="1px" border-top-color="#a0a0a0">
                                        ...
                                        <fo:table-cell number-columns-spanned="{$spanned_col}">                        
                                            <fo:block margin-left="3px">
                                                <xsl:value-of select="user/table_title"></xsl:value-of>
                                            </fo:block>
                                        </fo:table-cell>
                                    </fo:table-row>
                                    <fo:table-row font-size="11pt" background-color="#5ab8ec" height="20px" line-height="10px" font-weight="bold" border-top-width="1px" border-top-color="#a0a0a0" border-bottom-width="1px" border-bottom-color="#fad7a5" alignment-baseline="middle">
                        
                                        <xsl:variable name="column_count" select="count(//header/element)"/>
                                        <xsl:for-each select="//header/element">
                                            <fo:table-cell border-style="solid" border-width="0.007in" border-color="#f4f4f4" border-top-color="#a0a0a0">
                                                ...
                                                <xsl:if test="position() = 1">
                                                    <xsl:attribute name="border-left-color">#a0a0a0</xsl:attribute>
                                                </xsl:if> 
                                                <fo:block margin-left="3px">
                                ...
                                <fo:table-body>
                                    <xsl:for-each select="data/element">
                                        <xsl:variable name="spanned_col" select="count(//header/element)"/>
                                        <fo:table-row font-weight="bold" height="20px" line-height="20px"  background-color="#e5e5e5">
                                            
                                            <fo:table-cell number-columns-spanned="{$spanned_col}"  text-indent="10px"  border-style="solid" border-width="0.007in" border-color="#f4f4f4" border-bottom-color="#a0a0a0" border-right-color="#a0a0a0" border-left-color="#a0a0a0">
                                                <fo:block margin-left="3px">
                                        ...
                                        <xsl:for-each select="groupData/element">
                                            ...
                                            <fo:table-row height="20px" line-height="20px"  background-color="#e5e5e5">
                                                ...
                                                <xsl:for-each select="*">
                                                    <fo:table-cell border-style="solid" border-width="0.007in" border-color="#f4f4f4" border-bottom-color="#a0a0a0">
                                                        ...
                                                        <fo:block text-indent="10px" margin-left="7px">
                                                            <xsl:value-of select="." />
                                                        </fo:block>
                                                    </fo:table-cell>
                                                </xsl:for-each>                    
                        ...
                        <xsl:for-each select="graphs/element">
                            <fo:block-container margin-top="10px" text-align="center">
                                <xsl:variable name="content_path" select="content"/>                                    
                                <fo:block font-size="7pt" background-color="#fbfbfb">
                                    <fo:external-graphic src="{$content_path}" clear="none" float="left" content-height="4.83in" scaling="uniform" border="0" padding="0" margin="0" alignment-baseline="middle"/>
                                </fo:block>                                      
                            </fo:block-container>
                        </xsl:for-each>      
